...
void test_force_creator() {
    ...
    for (int i = 0; i < STEPS; i++) {
        vector_t expected_x = vec_rotate(radius, OMEGA * i * DT, VEC_ZERO);
...
void test_reaping() {
    scene_t *scene = scene_init();
    for (int i = 0; i < 3; i++) {
        scene_add_body(scene, body_init(make_shape(), 1, (rgb_color_t) {0, 0, 0}), 0);
    }
