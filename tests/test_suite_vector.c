...
void test_vec_rotate() {
    // No rotation
    assert(vec_isclose(vec_rotate((vector_t) {5, 7}, 0, VEC_ZERO), (vector_t) {5, 7}));
    // 90-degree rotation
    assert(vec_isclose(vec_rotate((vector_t) {5, 7}, 0.5 * M_PI, VEC_ZERO), (vector_t) {-7, 5}));
    // 180-degree rotation
    assert(vec_isclose(vec_rotate((vector_t) {5, 7}, M_PI, VEC_ZERO), (vector_t) {-5, -7}));
    // 270-degree rotation
    assert(vec_isclose(vec_rotate((vector_t) {5, 7}, 1.5 * M_PI, VEC_ZERO), (vector_t) {7, -5}));
    // 3-4-5 triangle
    assert(vec_isclose(vec_rotate((vector_t) {5, 0}, acos(4.0 / 5.0), VEC_ZERO), (vector_t) {4, 3}));
    // Rotate (0, 0)
    assert(vec_isclose(vec_rotate(VEC_ZERO, 1.0, VEC_ZERO), VEC_ZERO));
}
