...
void test_list_size1() {
    ...
    *v = VEC_ZERO;
    ...
    vector_t* v1 = list_get(l, 0);
    assert(vec_equal(*v1, (vector_t) {1, 1}));
    // Modify
    *v = (vector_t) {1, 2};
    assert(list_size(l) == 1);
    vector_t* v2 = list_get(l, 0);
...
void test_list_small() {
    ...
    *v = VEC_ZERO;
    ...
    vector_t* v1 = list_get(l, 0);
    vector_t* v2 = list_get(l, 1);
    vector_t* v3 = list_get(l, 2);

    assert(vec_equal(*v1, VEC_ZERO));
    assert(vec_equal(*v2, (vector_t) {1, 1}));
    assert(vec_equal(*v3, (vector_t) {2, 2}));
    ...
    vector_t* v4 = list_get(l, 3);
    vector_t* v5 = list_get(l, 4);
    assert(vec_equal(*v4, (vector_t) {3, 3}));
    assert(vec_equal(*v5, (vector_t) {4, 4}));
    ...
    v4 = list_get(l, 3);
    v5 = list_get(l, 4);
    assert(vec_equal(*v1, (vector_t) {0, 0}));
    assert(vec_equal(*v2, (vector_t) {1, 1}));
    assert(vec_equal(*v3, (vector_t) {2, 2}));
    assert(vec_equal(*v4, (vector_t) {5, 5}));
    assert(vec_equal(*v5, (vector_t) {6, 6}));
    ...
    *v5 = (vector_t) {8, 8};
    ...
    assert(vec_equal(*v2, (vector_t) {1, 1}));
    assert(vec_equal(*v3, (vector_t) {2, 2}));
    assert(vec_equal(*v4, (vector_t) {7, 7}));
    assert(vec_equal(*v5, (vector_t) {8, 8}));
...
void test_list_large_add_remove() {
    ...
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        vector_t* v = list_get(l, i);
        assert(vec_equal(*v, (vector_t) {i + 1, i + 1}));
    }
    list_free(l);
}
...
void test_out_of_bounds_access() {
    ...
    for (size_t size = 0; size <= max_size; size++) {
        ...
        for (access->index = 0; access->index < size; access->index++) {
            ...
            vector_t* v = list_get(access->list, access->index);
            *v = new_vector;
            vector_t* v1 = list_get(access->list, access->index);
            assert(vec_equal(*v1, new_vector));
        }
...
void remove_front_from_empty(void *l) {
    list_remove((list_t *) l, 0);
}

void test_empty_remove_front() {
    const size_t size = 100;
    list_t *l = list_init(size, (free_func_t) free);
    ...
    // Try removing from the empty list -- should fail an assertion
    assert(test_assert_fail(remove_front_from_empty, l));
...
void test_remove_front() {
    ...
    // Tests to see that it is removing from front and not back
    for (size_t i = 0; i < size; i++) {
        vector_t *list_v = list_remove(l, 0);
        assert(vec_equal(*list_v, (vector_t) {i, i}));
        free(list_v);
    }
    list_free(l);
}

void test_remove_anywhere() {
    const size_t size = 10;
    list_t *l = list_init(size, (free_func_t) free);
    ...
    for (size_t i = 0; i < 3; i++) {
        vector_t *list_v = list_remove(l, 2);
        assert(vec_equal(*list_v, (vector_t) {i + 2, i + 2}));
        free(list_v);
    }

    // Tests to see that it is removing from front and not back
    for (size_t i = 0; i < 3; i++) {
        vector_t *list_v = list_remove(l, 4);
        assert(vec_equal(*list_v, (vector_t) {i + 7, i + 7}));
        free(list_v);
    }

    vector_t *list_v = list_remove(l, 2);
    assert(vec_equal(*list_v, (vector_t) {5, 5}));
    free(list_v);

    vector_t *list_v1 = list_remove(l, 1);
    assert(vec_equal(*list_v1, (vector_t) {1, 1}));
    free(list_v1);

    vector_t *list_v2 = list_remove(l, 1);
    assert(vec_equal(*list_v2, (vector_t) {6, 6}));
    free(list_v2);

    vector_t *list_v3 = list_remove(l, 0);
    assert(vec_equal(*list_v3, (vector_t) {0, 0}));
    free(list_v3);
...
int main(int argc, char *argv[]) {
    ...
    DO_TEST(test_empty_remove_front)
    DO_TEST(test_remove_front)
    DO_TEST(test_null_values)
    DO_TEST(test_remove_anywhere)
